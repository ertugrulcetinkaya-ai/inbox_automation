...
- When multiple calendar payloads share the aggregate ICS limit, only complete `VCALENDAR` blocks that fit are retained; no valid block is cut at an arbitrary byte boundary. Recurring master invites are not expanded through `RRULE`/`RDATE`/`EXDATE` in v1; concrete mailbox occurrences and lifecycle updates are supported.
